...
export function JourneyCard({ journey, onUpdateStage, onStart, onDelete, onClick, onOpenInVSCode }: JourneyCardProps) {
  ...
  return (
    <div
      className={`bg-gray-800 border border-gray-700 rounded-lg p-4 hover:border-gray-600 transition-colors ${onClick ? 'cursor-pointer' : ''}`}
      onClick={onClick}
    >
      {/* Main row layout */}
          ...
          <div className="flex items-center gap-4 text-xs text-gray-400">
            ...
            {isStarted && journey.branch_name && (
              <div className="flex items-center gap-1.5 flex-shrink-0">
                ...
                <span className="text-gray-300 font-mono truncate max-w-[200px]" title={journey.branch_name}>
                  {journey.branch_name}
                </span>
              </div>
            )}
